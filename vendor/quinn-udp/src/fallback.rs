...
impl UdpSocketState {
    ...
    pub fn recv(
        ...
        // ffrwd patch: socket2 has no vectored recv on wasi; one buffer.
        // Upstream uses `recv_from_vectored(bufs)`, which does not exist there.
        let (len, addr) = socket.0.recv_from(&mut bufs[0])?;
        meta[0] = RecvMeta {
            len,
            stride: len,
...
fn send(socket: UdpSockRef<'_>, transmit: &Transmit<'_>) -> io::Result<()> {
    // ffrwd patch: discard the sent-byte count so the wasi build type-checks.
    socket.0
        .send_to(
            transmit.contents,
