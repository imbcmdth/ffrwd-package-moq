...
-- variables: source (input media path), relay (relay URL, host by name or IP), broadcast (broadcast path), rungs (how many rungs, matching the two lists), widths (comma list of rung widths, e.g. 1920,1280,854), bitrates (comma list of per-rung bitrates, e.g. 6000k,3000k,1000k), cert (a private relay's certificate, DER as hex; leave unset for a public relay), token (an auth token the relay demands; leave unset if it demands none), rows (what the sink reports: summary, groups or none), audio_group_ms (how long an audio group runs in milliseconds; leave unset for the default tenth of a second)
