...
// ffrwd patch: tokio's net module does not exist on wasm targets.
...
impl ClientBuilder {
    ...
    // ffrwd patch: wasi has no OS trust store, so system roots cannot
    // apply there; bring roots explicitly (e.g. with_server_certificates).
    #[cfg(not(target_os = "wasi"))]
...
impl Client {
    ...
    pub async fn connect(
        ...
        {
            // ffrwd patch: no DNS on wasi (tokio::net is absent there);
...
// ffrwd patch: the default client loads system roots, which wasi lacks.
#[cfg(all(any(feature = "aws-lc-rs", feature = "ring"), not(target_os = "wasi")))]
