...
--
-- relay names the relay by URL, its host an IP literal - the module
-- runs under a UDP grant alone, with no name lookup. cert is a private
-- relay's own certificate, DER as hex - a public value, not a secret;
-- left empty, the webpki roots baked into the module decide, which is
-- what a public relay's certificate chains to. The init segment rides
-- its own track, <track>.init.
