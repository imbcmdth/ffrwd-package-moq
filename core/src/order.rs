...
//! group first, so a reader that joins a broadcast already running is
//! handed a whole backlog in something close to reverse. What comes
//! out of this package is packets in DECODE ORDER - `ffrwd:av` says a
//! pad's packets are in decode order, and the stream-copy muxers a
//! query writes through (nut, matroska, mp4) refuse a timestamp that
//! goes backwards - so the reordering has to be absorbed here.
//!
//! [`Queue`] is that hold: groups in as they land, groups out in
//! sequence, and a count of everything it could not do. It decides
//! nothing about media; [`crate::group`] is where a group begins and
//! ends, and the demuxing is the caller's.
...
/// lines instead, one object to a line. `kind` says which of the three
/// shapes it is: a TRACK row every [`REPORT_EVERY`] and once more when
/// the track ends (`final`), a HOLE row for every hole given up on, and
/// a LATE row for every group that arrived below the cursor and could
/// not be used.
pub const ROWS_SCHEMA: &str = r#"{"type":"object","properties":{"kind":{"type":"string","enum":["track","hole","late"]},"track":{"type":"string"},"received":{"type":"integer"},"delivered":{"type":"integer"},"repeated":{"type":"integer"},"bytes":{"type":"integer"},"holes_opened":{"type":"integer"},"holes_filled":{"type":"integer"},"holes_abandoned_gone":{"type":"integer"},"holes_abandoned_budget":{"type":"integer"},"holes_abandoned_restart":{"type":"integer"},"holes_abandoned_end":{"type":"integer"},"dropped_late":{"type":"integer"},"fetches":{"type":"integer"},"fetches_refused":{"type":"integer"},"hold_max_groups":{"type":"integer"},"hold_max_bytes":{"type":"integer"},"reorder_max":{"type":"integer"},"first":{"type":["integer","null"]},"last":{"type":["integer","null"]},"final":{"type":"boolean"},"from":{"type":"integer"},"to":{"type":"integer"},"reason":{"type":"string","enum":["gone","budget","restart","end"]},"held_groups":{"type":"integer"},"held_bytes":{"type":"integer"},"group":{"type":"integer"},"cursor":{"type":"integer"}},"additionalProperties":false}"#;
...
/// joining a broadcast that has been running takes a whole backlog at
/// once, hundreds of groups deep, and the group it is waiting for may
/// be the last one on the wire. The hold therefore waits as long as the
...
/// How long a reader that has just joined waits for a lower group
/// sequence before it fixes its cursor.
///
...
/// `delivered` plus `dropped_late` plus what the hold still has.
...
pub struct Queue {
	...
	/// when it last fell; see [`JOIN_SETTLE`].
...
/// A group that arrived after its place in the sequence had passed.
#[derive(Serialize)]
struct LateRow<'a> {
	kind: &'static str,
	track: &'a str,
...
impl Queue {
	...
	pub fn push(&mut self, sequence: u64, frames: Vec<Bytes>) {
		...
		if self.cursor.is_none() && self.join_low.is_none_or(|(low, _)| sequence < low) {
	...
	pub fn take(&mut self, hold: Hold, last: bool, ask: &mut Vec<u64>) -> Option<(u64, Vec<Bytes>)> {
		...
		let ready = match self.cursor {
			// The join. The cursor is fixed at the lowest sequence the
...
mod tests {
	...
	fn a_reversed_backlog_goes_out_in_order() {
		...
		let mut queue = Queue::new("a0".into());
		for sequence in (1..200).rev() {
			queue.push(sequence, group(sequence));
	...
	fn the_end_of_a_track_takes_what_is_left_in_sequence() {
		let mut queue = Queue::new("a0".into());
		queue.push(5, group(5));
		queue.push(7, group(7));
